...
public class OrderRepository {
    ...
    public List<Order> getAll() {
        String sql = "select o.id, o.user_id, a.account_name, o.total_amount, o.status, o.payment_method ,o.order_time " +
                "from Orders o " +
                "left join Account a " +
                "on o.user_id = a.id " +
                "order by o.id desc ";
    ...
    public Order findOrderById(int id){
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                return new Order(rs.getInt("id"), rs.getInt("user_id"), rs.getString("account_name"), rs.getBigDecimal("total_amount"), rs.getString("status"), rs.getString("payment_method"), rs.getTimestamp("order_time"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Order> getOrder(String roleCheckStr, String roleStr, String fromPrice, String toPrice, String statusCheckStr, List<String> statuses, LocalDate fromDate, LocalDate toDate){
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()){
                ...
                System.out.println("getOrder: " + id + " " + userId + " " + accountName + " " + totalAmount + " " + status + " " + orderTime);
                Order order = new Order(id, userId, accountName, totalAmount, status, paymentMethod, orderTime);
                ans.add(order);
            }
